...
const useResultStylesheet = <DefaultStylesType>({
  ...
  styles,
  mergeStyles,
}: tUseResultStylesheetParams<DefaultStylesType>): DefaultStylesType =>
  useMemo<DefaultStylesType>(() => {
    const newStyles = Object.assign({}, defaultStyles, styles);
    const styleSheet: tAnyObject = newStyles;
    if (mergeStyles) {
      Object.entries(mergeStyles).forEach(entry => {
        const [key, val] = entry;
        styleSheet[key] = Object.assign({}, styleSheet[key], val);
      });
    }
    return newStyles;
  }, [defaultStyles, styles, mergeStyles]);

...
const TaskFormDurationField: React.FC = ({ children }) => {
    ...
    const warnings = useWarnings<iTaskFormCreate>(context.values, taskFormWarningManager);
    ...
    return (
        <FieldLabel label='Task duration'>
            ...
            {allowWarning ? <Text>{currentWarningDuration || currentWarningHours}</Text> : null}
        </FieldLabel>
    )
}
